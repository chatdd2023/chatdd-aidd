...
drug_file_path = 'F:\\work\\data\\drug_syn_smiles_patents_1102.json'
                ...
                if isinstance(properties['patents'], list):
                    for patents_properties in properties['patents']:
                        ...
                        result = mysqlhelper.insertone(
                            "insert into pharmolix_data_drugs_patents (drug_generic_name, patent_number, country, approved_date, expires_date, pediatric_extension, summary) values(%s, %s, %s, %s, %s, %s, %s)",
                            (name, val_number, val_country, val_approved, val_expires, val_pediatric, val_summary))
                        if result:
                            insert_count += 1
